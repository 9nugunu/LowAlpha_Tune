...
def get_args(argv: list[str] | None = None):
    parser = argparse.ArgumentParser(description="Scan AlphaC and Delta parameters (EQUILIBRIUM REGIME).")
...
CHECK_FILE_NAME = "check_eq.ele"  # <-- CHANGED: equilibrium-regime tracking
...
def run_single_check(rootname: str, work_dir: Path) -> bool:
    """Run check_eq.ele for a specific rootname and return success."""
...
def run_checks(rootnames: list[str], work_dir: Path) -> None:
    """Run check_eq.ele on each lattice result in parallel with verification."""
    print(f"Starting parallel checks for {len(rootnames)} results...")
    print(f"(Note: equilibrium regime is ~100x slower than induced regime)")
